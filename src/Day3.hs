module Day3
    (
        Point, Segment,
        segments, normalize, intersection, intersections, distance, delay, shortestDelay,
        run
    ) where
...
direction :: String -> (Int,Int) -- This is not technically a point, more like a vector
direction d = case head d of
        'R' -> ( 1, 0)
        'L' -> (-1, 0)
        'U' -> ( 0, 1)
        'D' -> ( 0,-1)
...
intersections wire1 wire2 =
    let s1 = map normalize wire1
        s2 = map normalize wire2 in
    let r = [(a,b) | a <- s1, b <- s2] >>= intersection
    in filter  (/=(0,0)) r
...
delay segments point = 
    let next acc (p1,p2) = case acc of 
            ...
            Right v ->  if fst p1 == fst p2 then
                            if fst p1 == fst point && snd p1 <= snd point && snd point <= snd p2 then
                                Left (v + snd point - snd p1)
                            else if fst p1 == fst point && snd p2 <= snd point && snd point <= snd p1 then
                                Left  (v + snd p1 - snd point)
                            else
                                Right (v + abs (snd p2 - snd p1))
                        else -- implicitly snd p1 == snd p2 (segments are either horizontal or vertical)
                            if  snd p1 == snd point && fst p1 <= fst point && fst point <= fst p2 then
                                Left (v + fst point - fst p1)
                            else if  snd p1 == snd point && fst p2 <= fst point && fst point <= fst p1 then
...
shortestDelay :: String -> String -> Int
shortestDelay wire1 wire2 = 
    let sw1 = segments wire1
        sw2 = segments wire2
    in head (sort (map (\p -> delay sw1 p + delay sw2 p) (intersections sw1 sw2)))


run :: IO ()
run = 
    let w1 = wiresStrings !! 0
        w2 = wiresStrings !! 1 in
    do
        print ("puzzle 1:" ++ show (distance w1 w2))
        print ("puzzle 2:" ++ show (shortestDelay w1 w2))