...
s2 :: State
s2 = State {memory = list2array [42,0,4,0,99], ip = 2, input = [], output = Nothing}

s3 :: State
s3 = State {memory = list2array [42,0,4,0,99], ip = 4, input = [], output = Just 42}
...
spec = do
    ...
    describe "nextStep" $ do
        ...
        it "ends the program" $ do
            (nextStep (State (list2array [99]) 0 [] Nothing)) `shouldBe` (State (list2array [99]) (-1) [] Nothing)
        it "processes an addition" $ do
            (nextStep (State (list2array [1,0,0,0]) 0 [] Nothing)) `shouldBe` (State (list2array [2,0,0,0]) 4 [] Nothing)
        it "processes an addition with immediate access" $ do
            (nextStep (State (list2array [1101,100,-1,4,0]) 0 [] Nothing)) `shouldBe` (State (list2array [1101,100,-1,4,99]) 4 [] Nothing)
        it "processes a multiplication" $ do
            (nextStep (State (list2array [2,3,0,3,99]) 0 [] Nothing)) `shouldBe` (State (list2array [2,3,0,6,99]) 4 [] Nothing)
        it "processes a multiplication with immediate access" $ do
            (nextStep (State (list2array [1002,4,3,4,33]) 0 [] Nothing)) `shouldBe` (State (list2array [1002,4,3,4,99]) 4 [] Nothing)
        it "processes a jump if true" $ do
            (nextStep (State (list2array [1105,4,7]) 0 [] Nothing)) `shouldBe` (State (list2array [1105,4,7]) 7 [] Nothing)
