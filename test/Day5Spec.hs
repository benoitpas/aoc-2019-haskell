...
spec = do
    ...
    describe "allSteps"  $ do
        prop "comparison 1" $
            \i -> output (allSteps (State (list2array [3,9,8,9,10,9,4,9,99,-1,8]) 0 i Nothing))  `shouldBe` Just (if i == 8 then 1 else 0)
        prop "comparison 2" $
            \i -> output (allSteps (State (list2array [3,9,7,9,10,9,4,9,99,-1,8]) 0 i Nothing))  `shouldBe` Just (if i < 8 then 1 else 0)
        prop "comparison 3" $
            \i -> output (allSteps (State (list2array [3,3,1108,-1,8,3,4,3,99]) 0 i Nothing))  `shouldBe` Just (if i == 8 then 1 else 0)
        prop "comparison 4" $
            \i -> output (allSteps (State (list2array [3,3,1107,-1,8,3,4,3,99]) 0 i Nothing))  `shouldBe` Just (if i < 8 then 1 else 0)
        prop "comparison 5" $
    ...
    describe "runProgram" $ do
        prop "simple input/output test" $
            \i -> (runProgram "3,0,4,0,99" i) `shouldBe` Just (i::Integer)
